...
class TargetViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    func ButtonAction(_ sender: UIButton) {
        ...
        if sender.titleLabel?.text == "查看更多数据" {
            ...
            let resultVC:ResultViewController = UIStoryboard.init(name: "Main", bundle: nil).instantiateViewController(withIdentifier: "ResultViewController") as! ResultViewController
            ...
            let navResultVC:UINavigationController = UINavigationController.init(rootViewController: resultVC)
                self.present(navResultVC, animated: true, completion: {
            })
//            var bbbbbb:[Int]! = []
//            for i in 0...nbrsArrow-1 {
        ...
        if let dvc = segue.destination as? UINavigationController{
            if let identifier = segue.identifier {
                ...
                case "targetToResult":
                    ...
                    nvc.xStr = xFloat
                    nvc.tenStr = tenFloat
                    nvc.nineStr = nineFloat
                    nvc.eightStr = eightFloat
                    nvc.sevenStr = sevenFloat
                    nvc.sixStr = sixFloat
                    nvc.fiveStr = fiveFloat
                    nvc.fourStr = fourFloat
                    nvc.threeStr = threeFloat
                    nvc.twoStr = twoFloat
                    nvc.oneStr = oneFloat
                    nvc.MStr = mFloat
