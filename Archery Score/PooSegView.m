...
-(void)initUI
{
    ...
    if (setLines) {
        int a = 1;
        int b = [[NSString stringWithFormat:@"%ld",titlesArr.count] intValue];
        int c = b - a;
...
-(void)btnTap:(UIButton *)sender
{
    ...
    sender.selected = YES;
    selectedBtn = sender;
    if ([self.delegate respondsToSelector:@selector(didSelectedSegmentAtIndex:)]) {
        [self.delegate didSelectedSegmentAtIndex:sender.tag];
    }
}
@end
