            ...
            if (semaphoreWait != 0) {
                ...
                if (self->_activity == kCFRunLoopBeforeSources || self->_activity == kCFRunLoopAfterWaiting) {
                    ...
                    if (++strongSelf.countTime < strongSelf.standstillCount){
                        PNSLog(@"%ld",strongSelf.countTime);
                        continue;
                    }
                    [strongSelf logStack];
                    ...
                    [[YZLogFile sharedInstance] writefile:backtrace withAPIBase:self.apiBase];
                    
                    if (strongSelf.canUpload)
                    {
                        [YZLogFile sharedInstance].canUploadBlock = strongSelf.canUpload;
                    }
...
-(void)uploadFile
{
    [[YZLogFile sharedInstance] update];
}
