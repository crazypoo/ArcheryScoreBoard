...
- (void)CodeTypeImageView{
    ...
    [self addSubview:({
        ...
        [self.refresh setImage:kImageNamed(@"image_refresh") forState:UIControlStateNormal];
...
- (void)CodeTypeLabelView{
    ...
    [self addSubview:({
        self.mainImage.frame = CGRectMake(margin, CGRectGetMaxY(self.tipLabel.frame)+margin, self.width-margin*2, imageHeight);
        self.mainImage.image = [CGBaseGobalTools image:self.name compatibleWithTraitCollection:self.traitCollection];
        self.mainImage.contentMode =  UIViewContentModeScaleAspectFill;
        self.mainImage.clipsToBounds = YES;
...
//添加可移动的图片
- (void)addMoveImage{
    UIImage *normalImage = [CGBaseGobalTools image:self.name compatibleWithTraitCollection:self.traitCollection];
...
- (void)setMyTipLabetText{
    ...
    [attStr addAttribute:NSFontAttributeName value:AppFontNormal range:NSMakeRange(0, str.length)];
    [attStr addAttribute:NSFontAttributeName value:AppFontNormal range:[str rangeOfString:self.factChinese]];
...
- (void)successShow{
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        [[CGBaseGobalTools getCurrentVC] presentViewController:alert animated:YES completion:nil];
    });
...
- (WMZSlider *)slider{
    if (!_slider) {
        ...
        _slider.thumbTintColor = [CGBaseGobalTools AppWhite];
    }
    return _slider;
}
...
- (UILabel *)label{
    if (!_label) {
        ...
        _label.font = AppFontNormal;
        _label.textAlignment = NSTextAlignmentCenter;
        _label.textColor = [CGBaseGobalTools AppOrange];
        _label.layer.masksToBounds = YES;
        _label.layer.borderWidth = 1;
        _label.layer.borderColor = [CGBaseGobalTools AppOrange].CGColor;
    }
    return _label;
}
